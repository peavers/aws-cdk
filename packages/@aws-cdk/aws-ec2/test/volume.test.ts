...
describe('volume', () => {
  ...
  test('validation iops', () => {
    ...
    for (const volumeType of [
      ...
      }).toThrow('`iops` may only be specified if the `volumeType` is `PROVISIONED_IOPS_SSD`/`IO1`');
    }
    ...
    }).toThrow('`iops` must be in the range 100 to 64,000, inclusive.');
    expect(() => {
      new Volume(stack, `Volume${idx++}`, {
        availabilityZone: 'us-east-1a',
        size: cdk.Size.gibibytes(10),
    ...
    }).toThrow('`iops` has a maximum ratio of 50 IOPS/GiB.');
  ...
  test('validation multi-attach', () => {
    ...
    for (const volumeType of [
      ...
      }).toThrow('multi-attach is supported exclusively on `PROVISIONED_IOPS_SSD` volumes.');
    }
  ...
  test('validation size in range', () => {
    ...
    for (const testData of [
      ...
      }).toThrow(`\`${volumeType}\` volumes must be between ${min} GiB and ${max} GiB in size.`);
      expect(() => {
        new Volume(stack, `Volume${idx++}`, {
          availabilityZone: 'us-east-1a',
